...
private let logger = Logger(label: "OSXQuery.ElementSearch")
...
private func resetTraversalState() {
    ...
    traversalDeadline = Date().addingTimeInterval(osxqueryTraversalTimeout)
}
    ...
    maxDepth: Int)
{
       ...
       (element.role().map { containerRoles.contains($0) } ?? false)
    {
        ...
        if let deadline = traversalDeadline, Date() > deadline {
            logger.warning("Traverse: global search timeout (\(osxqueryTraversalTimeout)s) reached. Aborting traversal.")
            return
        }
...
/// Default timeout (seconds) for a full tree traversal. Override at runtime by setting `osxqueryTraversalTimeout`.
public nonisolated(unsafe) var osxqueryTraversalTimeout: TimeInterval = 30

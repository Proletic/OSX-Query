...
/// The main class for OSXQuery accessibility automation operations.
///
/// OSXQuery provides a comprehensive interface for interacting with macOS accessibility APIs.
/// It supports querying UI elements, performing actions, extracting text, and batch operations.
///
/// ## Usage
///
/// ```swift
/// let osxquery = OSXQuery.shared
/// let command = AXCommandEnvelope(commandID: "test", command: .query(queryCommand))
/// let response = osxquery.runCommand(command)
/// ```
///
/// ## Topics
///
...
/// - ``AXResponse``
@MainActor
public class OSXQuery {
    // MARK: Lifecycle

    /// Creates a new OSXQuery instance.
    @MainActor public init() {}

    // MARK: Public

    /// The shared singleton instance of OSXQuery.
    ///
    ...
    public static let shared = OSXQuery()

    /// Executes an accessibility command and returns the response.
    ///
    /// This is the central method for all OSXQuery operations. It processes
    ...
    /// )
    /// let response = OSXQuery.shared.runCommand(envelope)
    /// ```
